...
# mise — project tool manager. Activated after asdf so mise wins the
# PATH race for repos that use `.tool-versions` (e.g. remote-vibed-labs
# pins elixir 1.19.4-otp-27 + erlang 27.3.3 — versions mise has
# installed but asdf does not). Without this, `mix` runs through the
# asdf shim and fails with "no version is set for command mix".
